  ...
  template: `
    <app-glass-wrapper panelClass="h-[67vh] bg-transparent backdrop-blur-[25px] p-3 sm:p-4 lg:p-5 rounded-2xl border border-white/10 shadow-2xl overflow-hidden max-h-[85vh] w-full max-w-none">
      <div class="w-full h-full overflow-y-auto overflow-x-hidden custom-scrollbar pr-0.5 md:pr-2 flex flex-col text-white">
        ...
        <div class="relative border-l border-white/30 ml-1.5 md:ml-3 pl-5 md:pl-7 py-2 flex-1 flex flex-col justify-between gap-5 md:gap-6">
          <div *ngFor="let edu of education" class="relative group transition-transform duration-300 hover:translate-x-1">
            ...
            <div class="border border-white/15 bg-white/5 rounded-2xl p-3 md:p-4 transition-colors hover:border-white/25 hover:bg-white/10">
              <div class="flex flex-col sm:flex-row items-start gap-3 md:gap-4">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="font-bold text-sm md:text-base text-white/90 italic">{{ edu.field }}</p>
                  <a *ngIf="edu.link" [href]="edu.link" target="_blank" class="inline-flex items-center gap-1.5 text-xs md:text-sm font-bold uppercase text-white/95 hover:text-cyan-100 transition-colors mt-1">
                  ...
                  <p *ngIf="!edu.link" class="text-xs md:text-sm font-bold uppercase text-white/95 mt-1">{{ edu.institution }}</p>
                </div>
              </div>
              <p class="mt-3 text-sm md:text-base leading-relaxed text-white/95">{{ edu.description }}</p>
              <p *ngIf="edu.metaNote" class="mt-1.5 text-xs md:text-sm leading-relaxed text-white/80 italic">{{ edu.metaNote }}</p>
              <div class="h-px bg-white/20 my-3"></div>
              <div class="flex flex-col gap-3">
                <div>
                  <h4 class="text-xs font-bold uppercase tracking-widest text-cyan-100 drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)] mb-2">Core Coursework</h4>
                  <div class="flex flex-wrap gap-1.5">
                    <span *ngFor="let course of edu.courseHighlights" class="rounded-full border border-cyan-100/30 bg-white/[0.06] px-2 py-1 text-[10px] md:text-[11px] font-medium text-white/95">
                      {{ course }}
                    </span>
                  </div>
                </div>
